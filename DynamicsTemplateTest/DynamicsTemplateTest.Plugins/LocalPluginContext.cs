...
namespace DynamicsTemplateTest.Plugins
{
    /// <summary>
    ...
    public class LocalPluginContext
    {
        ...
        public T GetInputParameter<T>(string inputParameterName)
        {
            if (PluginExecutionContext.InputParameters.Contains(inputParameterName))
            {
                return (T)PluginExecutionContext.InputParameters[inputParameterName];
            }

            return default(T);
        }

        public void SetOutputParameter<T>(string outputParameterName, T outputParameterValue)
        {
            if (PluginExecutionContext.OutputParameters.Contains(outputParameterName))
            {
                PluginExecutionContext.OutputParameters[outputParameterName] = outputParameterValue;
            }
        }
    }
}
